...
namespace hypeJunction\Faker\Tests;

use PHPUnit\Framework\TestCase;

/**
 * Migration regression guard for plugin "hypefaker" (target Elgg 7.x).
 *
 * Seeded from elgg-test-writer's MigrationRegressionTest template and scoped to
 * hypefaker's own migrated code. NOTE: the bundled mod/hypeApps sub-package is a
 * separate legacy framework (own composer.json + vendors/) that is NOT part of
 * this plugin's migration surface, so files() excludes it (see the exclusion
 * regex) — otherwise its un-migrated calls (forward/register_error/…) would
...
final class MigrationRegressionTest extends TestCase
{
    ...
    private const REMOVED_FUNCTIONS = [
        ...
        7 => [
            ...
            // hypefaker migration-fix symbols (refs e3b4875, ed59bd5, 0b811c5):
            // removed/renamed core procedural APIs the gen_* actions used to call.
            'get_write_access_array',
            'elgg_get_country_info',
            'notify_user',
    ...
    private static function files(string $sub, string $ext): array
    {
        ...
        foreach ($it as $f) {
            $path = $f->getPathname();
            // Exclude dependency dirs AND the bundled mod/ sub-package
            // (mod/hypeApps is a self-contained legacy framework, not part of
            // hypefaker's own migration surface).
    ...
    public function testNoCamelCasePluginIdCallsites(): void
    {
        ...
        // The plugin id sits at a DIFFERENT argument position in each of these
        // functions (verified against Elgg 7.0 engine/lib/plugins.php):
        //   elgg_get_plugin_from_id(string $plugin_id)                                -> arg 1
        //   elgg_get_plugin_setting(string $name, string $plugin_id, $default)         -> arg 2
        //   elgg_get_plugin_user_setting(string $name, int $user_guid,
        //                                string $plugin_id, $default)                  -> arg 3
        // Scanning the whole call for any uppercase string literal misidentifies
        // sibling arguments as the plugin id — e.g.
        //   elgg_get_plugin_setting('locale', 'hypefaker', 'en_US')
        // where 'en_US' is the default Faker locale, not a plugin id.
        $str = '[\'"]([A-Za-z0-9_.-]+)[\'"]';                                  // a literal string argument
        $any = '(?:\'[^\']*\'|"[^"]*"|[\w$>:\\\\-]+\s*\([^()]*\)|[^,()]+)';    // any other single argument
        $patterns = [
            "/elgg_get_plugin_from_id\s*\(\s*{$str}/",
            "/elgg_get_plugin_setting\s*\(\s*{$any},\s*{$str}/",
            "/elgg_get_plugin_user_setting\s*\(\s*{$any},\s*{$any},\s*{$str}/",
        ];
