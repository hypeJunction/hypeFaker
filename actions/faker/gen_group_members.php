...
foreach ($groups as $group) {
	...
	if (!$group->isPublicMembership()) {
		...
		foreach ($invitees as $invitee) {
			if (!(get_entity($invitee->guid)?->getRelationship($group->guid, 'member') ?? null)) {
		...
		foreach ($requestors as $requestor) {
			if (!(get_entity($group->guid)?->getRelationship($requestor->guid, 'invited') ?? null) && !(get_entity($requestor->guid)?->getRelationship($group->guid, 'member') ?? null)) {
