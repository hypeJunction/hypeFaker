...
global $messages_pm;
$messages_pm = 1;
$senders = new ElggBatch('elgg_get_entities', array('types' => 'user', 'metadata_names' => '__faker', 'limit' => 0));
foreach ($senders as $sender) {
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($success && $seccess_sent) {
            ...
            $body = elgg_echo('messages:email:body', array($sender->name, $message_contents, elgg_get_site_url() . "messages/inbox/" . $recipient->username, $sender->name, elgg_get_site_url() . "messages/compose?send_to=" . $sender_guid));
