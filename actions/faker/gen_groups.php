...
/**
 * Map plugin-setting access mode strings to ElggGroup constants.
 *
 * @param string $mode 'members_only' | 'unrestricted'
 * @return int|null
 */
...
foreach ([ACCESS_PRIVATE, ACCESS_LOGGED_IN, ACCESS_PUBLIC] as $visibility) {
	foreach ([hypefaker_get_group_content_access_mode('members_only'), hypefaker_get_group_content_access_mode('unrestricted')] as $content_access_mode) {
		foreach ([ACCESS_PRIVATE, ACCESS_PUBLIC] as $membership) {
			for ($i = 0; $i < $count; $i++) {
				...
				elgg_create_river_item(['view' => 'river/group/create', 'action_type' => 'create', 'subject_guid' => $owner->guid, 'object_guid' => $group->guid]);
...
if ($error) {
	elgg_register_success_message(elgg_echo('faker:gen_groups:error', [$success, $error]));
} else {
	elgg_register_success_message(elgg_echo('faker:gen_groups:success', [$success]));
}
