...
for ($i = 0; $i < $count; $i++) {
	...
	foreach ($containers as $container) {
		...
		if ($bookmark->save()) {
			$success++;
			elgg_create_river_item(['view' => 'river/object/bookmarks/create', 'action_type' => 'create', 'subject_guid' => $owner->guid, 'object_guid' => $bookmark->getGUID()]);
		} else {
			$error++;
		}
	}
...
if ($error) {
	elgg_register_success_message(elgg_echo('faker:gen_bookmarks:error', [$success, $error]));
} else {
	elgg_register_success_message(elgg_echo('faker:gen_bookmarks:success', [$success]));
}
