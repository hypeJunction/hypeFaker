...
for ($i = 0; $i < $count; $i++) {
	...
	foreach ($containers as $container) {
		...
		if ($file->save()) {
			...
			elgg_create_river_item(['view' => 'river/object/file/create', 'action_type' => 'create', 'subject_guid' => $file->owner_guid, 'object_guid' => $file->guid]);
		} else {
			$error++;
		}
	}
...
if ($error) {
	elgg_register_success_message(elgg_echo('faker:gen_files:error', [$success, $error]));
} else {
	elgg_register_success_message(elgg_echo('faker:gen_files:success', [$success]));
}
