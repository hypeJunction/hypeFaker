...
/**
 * Create a fake page entity owned by $owner inside $container.
 *
 * @param \ElggUser   $owner     User who owns the new page
 * @param \ElggEntity $container Container entity (group or user)
 * @param \ElggObject $parent    Optional parent page for nested pages
...
function hypefaker_add_page($owner, $container, $parent = null) {
	...
	if ($page->save()) {
		...
		elgg_create_river_item(['view' => 'river/object/page/create', 'action_type' => 'create', 'subject_guid' => $page->owner_guid, 'object_guid' => $page->getGUID()]);
		// add some revisions
		$users = elgg_get_entities(['types' => 'user', 'limit' => rand(1, 10), 'order_by' => 'RAND()', 'metadata_names' => '__faker']);
		...
		if ($parent) {
			...
			if ($children_count) {
				for ($i = 0; $i < $children_count; $i++) {
					$subparent = hypefaker_add_page($owner, $container, $parent);
					if ($subparent) {
						$success++;
...
if ($error) {
	elgg_register_success_message(elgg_echo('faker:gen_pages:error', [$success, $error]));
} else {
	elgg_register_success_message(elgg_echo('faker:gen_pages:success', [$success]));
}
